...
USING (user_id = auth.user_id() AND auth.is_merchant())
WITH CHECK (user_id = auth.user_id() AND auth.is_merchant());
...
USING (active = TRUE AND auth.is_verified());

-- Merchants can view their own listings (active + inactive)
CREATE POLICY "Merchants can view own listings"
ON listings FOR SELECT
USING (
  merchant_id IN (SELECT id FROM merchant_profiles WHERE user_id = auth.user_id()) 
...
WITH CHECK (
  merchant_id IN (SELECT id FROM merchant_profiles WHERE user_id = auth.user_id())
  AND auth.is_merchant()
  AND auth.is_verified() -- Must be verified to create listings
);
...
USING (client_id = auth.user_id() AND auth.is_client());
...
ON bookings FOR SELECT
USING (merchant_id = auth.user_id() AND auth.is_merchant());
...
USING (merchant_id = auth.user_id() AND auth.is_merchant())
WITH CHECK (merchant_id = auth.user_id() AND auth.is_merchant());

-- Clients can update bookings (cancel only, handled at app level)
CREATE POLICY "Clients can update own bookings"
ON bookings FOR UPDATE
USING (client_id = auth.user_id() AND auth.is_client())
WITH CHECK (client_id = auth.user_id() AND auth.is_client());
...
WITH CHECK (
  ...
  booking_id IN (
    ...
    WHERE client_id = auth.user_id() OR merchant_id = auth.user_id()
  )
);
...
-- SELECT * FROM listings WHERE merchant_id IN (SELECT id FROM merchant_profiles WHERE user_id = auth.user_id());

-- Test 4: Verify clients can only see their own bookings
-- Expected: Only bookings where client_id = current user's id
-- SELECT * FROM bookings WHERE client_id = auth.user_id();
...
-- SELECT * FROM bookings WHERE merchant_id = auth.user_id();
