...
export default function OnboardingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Profile created, now redirect to ID verification (hard gate)
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
              <p className="text-sm text-blue-900 font-medium mb-1">
                🔒 Next Step: ID Verification
              </p>
              <p className="text-xs text-blue-700">
                After completing this form, you'll need to verify your identity with a government-issued ID. 
                This is a security requirement for all users on My Place.
              </p>
            </div>

            <Button type="submit" className="w-full" size="lg" disabled={loading}>
              {loading ? 'Creating profile...' : 'Continue to ID Verification'}
            </Button>
          </form>
        </CardContent>
      </Card>
